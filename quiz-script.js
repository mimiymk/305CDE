    ...
    $.quiz = function(quiz_data, results_data, options) {
        ...
        var quiz = {
          ...
          //  not_finished_html : undefined,
            not_finished_html : "keep answering questions!",
            cheating : false,
            ...
            make_quiz_data_from_spreadsheet_data: function(tabletop) {
                ...
                for (i = 0; i < data.length; i++) {
                  ...
                  //          Math.round(Math.random() * possible_wrong_answers.length)
                          // Sort Possible Wrong Ansers First
                          Math.round(1 * possible_wrong_answers.length)
                        );
                    }
...
console.log("you got " + right_answers + "!");
                  //sendEval(right_answers);
                  sendEval(right_answers)
                }
    ...
    //  alert(quiz.init(quiz_data, results_data, options));
    };
...
function sendEval(score){
  //alert(score);

  alert("This is my score:" + score);
  var returnValue = score;
  
      // If test result is fail, encourage the user to use the consultation service     
      	if (returnValue < 7) {
           // $("#suggest").css("color", "red");
            greeting = "We recommend you to have a consultation with us! We will help you to build a healthy relationship!";
          } 
      // If test result is pass, recommend the user to explore other resources
          else {
          //  $("suggest").css("color", "green");
            greeting = "Congratulations! You get a healthy relationship! Explore our website to know how to keep the healthy relationship!";
          }
        $('#suggest').html(greeting);
  ...
  // Start	
  if (confirm("Do you confirm to send your evaluation result?")) {
    var user = {};
    var nCorrect = "";
    //user.action = "add_favourite";
    user.act = "add_evaluation";
    //user.url = $("#"+img).attr("src");
    user.score = returnValue;
    // y=nCorrect; alert(y)
    user.username = localStorage.getItem("username");

    $.ajax( {
      //url: "http://localhost:8000/index.html",
      url: "http://port-5001.mimi-mimiyu199137199.codeanyapp.com/search.html",
      type: "POST",
      data: user
    });
    return false;
